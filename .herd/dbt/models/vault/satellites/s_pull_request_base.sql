...
WITH source AS (
    SELECT
        ...
        pr_closed_at,
        is_deleted,
        {{ hashdiff(['pr_branch_name', 'pr_title', 'pr_lines_added', 'pr_lines_deleted', 'pr_files_changed', 'pr_merged_at', 'pr_closed_at', 'is_deleted']) }} AS pull_request_hd,
        load_ts,
        rsrc
...
{% if is_incremental() %}
LEFT JOIN (
    SELECT pull_request_tk, pull_request_hd
    FROM {{ this }}
    ...
    AND s.pull_request_hd = existing.pull_request_hd
